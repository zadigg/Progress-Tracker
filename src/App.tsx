...
import { CheckCircle2, Circle, GraduationCap, Plus, Book, X, RepeatIcon as ReactIcon, Footprints as SpringBoot, Angry as Angular, Code2, FileInput, Trash2, ChevronDown, ChevronRight, Menu, Move, GripVertical } from 'lucide-react';
...
function App() {
  ...
  const moveQuestion = (dragIndex: number, hoverIndex: number) => {
    const dragQuestion = filteredQuestions[dragIndex];
    ...
    setQuestions(updatedQuestions.map((q, index) => ({
      ...q,
      order: index + 1
    })));
  };
  ...
  const QuestionItem = ({ question, index, moveQuestion }) => {
    ...
    return (
      <div
        ref={ref}
        className={`bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex items-center gap-4 hover:shadow-md transition-all duration-200 animate-fade-in ${isDragging ? 'opacity-50' : 'opacity-100'}`}
      >
